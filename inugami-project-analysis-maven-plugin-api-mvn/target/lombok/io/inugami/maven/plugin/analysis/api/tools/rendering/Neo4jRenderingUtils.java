...
public final class Neo4jRenderingUtils {
    ...
    // API
    // =========================================================================
    public static String rendering(final Map<String, Collection<DataRow>> data, final ConfigHandler<String, String> configuration, final String context) {
    ...
    private static String renderTable(final Collection<DataRow> inputData) {
        ...
        if (inputData == null || inputData.isEmpty()) {
            ...
            int displaySize = columnsSize.entrySet().stream().mapToInt(Map.Entry::getValue).sum();
            ...
            for (final DataRow row : data) {
                ...
                for (final Map.Entry<String, Integer> header : columnsSize.entrySet()) {
                    ...
                    if (value == null) {
                        ...
                        final String renderedValue = processRenderingValue(value, cursor, columnsSize.get(header.getKey()));
                        writer.write(renderedValue);
                        writer.write(ConsoleColors.createLine(" ", columnsSize.get(header.getKey()) - renderedValue.length()));
                        writer.write(COLUMN_SEP);
                    }
                ...
                if (item != null) {
                    for (final Map.Entry<String, Serializable> entry : item.getProperties().entrySet()) {
                        ...
                        if (resultItem == null || resultItem < maxValueSize) {
                            result.put(entry.getKey(), value.length() < entry.getKey().length() ? entry.getKey().length() : maxValueSize);
                        }
                    }
                }
            }
    ...
    // EXPORT
    // =========================================================================
    private static void processExport(final Map<String, Collection<DataRow>> data, final ConfigHandler<String, String> configuration, final String context) {
    ...
    private static void writeCsvFile(final String key, final Collection<DataRow> values, final String context, final File targetDir) {
        if (values == null || values.isEmpty()) {
            return;
        }
        final List<DataRow> data = new ArrayList<>(values);
        final String filePath = new StringBuilder().append(targetDir.getAbsolutePath()).append(File.separator).append(context).append("_").append(key.replaceAll(" ", "")).append(".csv").toString();
    ...
    private static void writeRow(final DataRow row, final Set<String> headers, final JsonBuilder csv) {
