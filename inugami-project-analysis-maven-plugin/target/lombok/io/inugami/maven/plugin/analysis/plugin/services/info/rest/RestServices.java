...
public class RestServices implements ProjectInformation, QueryConfigurator {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RestServices.class);
    private static final List<String> QUERIES = List.of("META-INF/queries/search_consumers.cql", "META-INF/queries/search_produce.cql", "META-INF/queries/search_services_rest.cql");
    ...
    public ConfigHandler<String, String> configure(final String queryPath, final Gav gav, final ConfigHandler<String, String> configuration) {
        ...
        config.putAll(Map.ofEntries(Map.entry("groupId", gav.getGroupId()), Map.entry("artifactId", gav.getArtifactId()), Map.entry("version", gav.getVersion()), Map.entry("serviceType", "Rest")));
        return config;
    }
    ...
    public void process(final InfoContext context) {
        ...
        final DependencyRest consumeDependencies = searchConsumedService(gav, dao, context.getConfiguration());
        final DependencyRest exposedDependencies = searchExposedService(gav, dao, context.getConfiguration());
        renderDependencies(gav, consumeDependencies, exposedDependencies);
        dao.shutdown();
    }

    private DependencyRest searchConsumedService(final Gav gav, final DefaultNeo4jDao dao, final ConfigHandler<String, String> config) {
        //@formatter:off
        ...
        if (!resultSet.isEmpty()) {
            ...
            for (final Record record : resultSet) {
                final Map<RestEndpoint, DependencyRest> localeEndpoints = extractAllEndpoint(resultSet, this::extractConsumeEndpoint);
    ...
    private DependencyRest searchExposedService(final Gav gav, final DefaultNeo4jDao dao, final ConfigHandler<String, String> config) {
        //@formatter:off
        final DependencyRest result = new DependencyRest();
        ...
        if (!resultSet.isEmpty()) {
            final Map<RestEndpoint, DependencyRest> endpoints = new LinkedHashMap<>();
            for (final Record record : resultSet) {
                final Map<RestEndpoint, DependencyRest> localeEndpoints = extractAllEndpoint(resultSet, this::extractExposedEndpoint);
                if (localeEndpoints != null) {
                    localeEndpoints.entrySet().forEach((entry -> {
                        result.addProducers(entry.getValue().getProducers());
                        result.addConsumers(entry.getValue().getConsumers());
    ...
    private void renderDependencies(final Gav gav, final DependencyRest consumeDependencies, final DependencyRest exposedDependencies) {
        ...
        if (consumeDependencies != null && !consumeDependencies.getProducers().isEmpty()) {
            ...
            for (final String appExpose : consumeDependencies.getProducers()) {
                json.write(ConsoleColors.BLUE_BOLD).write(gav.getHash()).write(" -> ").write(appExpose).write(ConsoleColors.RESET);
            }
        }
        if (exposedDependencies != null && !exposedDependencies.getConsumers().isEmpty()) {
            ...
            for (final String appConsumer : exposedDependencies.getConsumers()) {
                json.write(ConsoleColors.YELLOW_BOLD).write(gav.getHash()).write(" <- ").write(appConsumer).write(ConsoleColors.RESET);
            }
        }
        json.line();
        json.line();
    ...
    // EXTRACT
    // =========================================================================
    private Map<RestEndpoint, DependencyRest> extractAllEndpoint(final List<Record> resultSet, final BiConsumer<Map<RestEndpoint, DependencyRest>, Map<String, Object>> handler) {
    ...
    private void extractConsumeEndpoint(final Map<RestEndpoint, DependencyRest> buffer, final Map<String, Object> queryNodes) {
        ...
        if (endpoint != null) {
            ...
            if (savedDependencies == null) {
                buffer.put(endpoint, new DependencyRest(appProducer == null ? null : appProducer.getName(), appConsumer == null ? null : appConsumer.getName()));
            } else {
    ...
    private void extractExposedEndpoint(final Map<RestEndpoint, DependencyRest> buffer, final Map<String, Object> queryNodes) {
    ...
    private String renderEndpoint(final RestEndpoint endpoint, final DependencyRest dependencies) {
        ...
        result.write("[").write(endpoint.getVerb()).write("] ").write(endpoint.getUri()).write(ConsoleColors.RESET).line();
        result.write("\tuid:").write(endpoint.getUid()).line();
        NodeUtils.processIfNotEmptyForce(endpoint.getMethod(), value -> result.write("\tMethod:").write(value).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getDescription(), value -> result.write("\tDescription:").write(value).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getHeaders(), value -> result.write("\tHeader:").write(value).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getConsume(), value -> result.write("\tAccept:").write(value).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getProduce(), value -> result.write("\tContent-Type:").write(value).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getBody(), value -> result.write("\trequest payload:\n").write(spacePayload(value, "\t\t")).line());
        NodeUtils.processIfNotEmptyForce(endpoint.getResponseType(), value -> result.write("\tresponse payload:\n").write(spacePayload(value, "\t\t")).line());
