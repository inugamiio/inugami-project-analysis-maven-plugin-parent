...
public class ReleaseNote implements ProjectInformation, QueryConfigurator {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ReleaseNote.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    private static final List<String> QUERIES = List.of(QUERIES_SEARCH_RELEASE_NOTE_SIMPLE_CQL, QUERIES_SEARCH_RELEASE_NOTE_FULL_CQL);
    ...
    public ConfigHandler<String, String> configure(final String queryPath, final Gav gav, final ConfigHandler<String, String> configuration) {
        ...
        config.putAll(Map.ofEntries(Map.entry(GROUP_ID, gav.getGroupId()), Map.entry(ARTIFACT_ID, gav.getArtifactId()), Map.entry(VERSION, gav.getVersion())));
        return config;
    }
    ...
    public void process(final InfoContext context) {
        ...
        final List<ReleaseNoteExtractor> extractors = SpiLoader.INSTANCE.loadSpiServicesByPriority(ReleaseNoteExtractor.class);
        for (final ReleaseNoteExtractor extractor : extractors) {
            try {
                extractor.extractInformation(releaseNoteResult, gav, previousGav, dao, replacements, context);
    ...
    // WRITE JSON
    // =========================================================================
    private void writeReleaseNote(final ReleaseNoteResult releaseNoteResult, final InfoContext context) {
    ...
    // RENDERING
    // =========================================================================
    private void rendering(final ReleaseNoteResult releaseNoteResult, final JsonBuilder writer, final ConfigHandler<String, String> configuration) {
    ...
    private void renderingMergeRequest(final List<MergeRequests> mergeRequests, final JsonBuilder writer, final ConfigHandler<String, String> configuration) {
    ...
    private void renderingIssues(final List<Issue> issues, final JsonBuilder writer, final ConfigHandler<String, String> configuration) {
        if (issues != null && !issues.isEmpty()) {
            ...
            for (final Issue issue : issues) {
                final Map<String, Serializable> properties = new LinkedHashMap<>();
                final DataRow row = new DataRow();
                final String labels = issue.getLabels() == null || issue.getLabels().isEmpty() ? "" : String.join(" ", issue.getLabels());
    ...
    private void renderingAuthors(final Set<Author> authors, final JsonBuilder writer, final ConfigHandler<String, String> configuration) {
    ...
    private List<Replacement> buildReplacements(final ConfigHandler<String, String> configuration) {
        ...
        if (config != null) {
            try {
                final List<ReplacementConfig> data = ObjectMapperBuilder.build().readValue(config, new TypeReference<List<ReplacementConfig>>() {
                });
                for (final ReplacementConfig replacementConfig : data) {
                    result.add(Replacement.builder().pattern(Pattern.compile(replacementConfig.getFrom())).replacement(replacementConfig.getTo()).build());
                }
            } catch (final JsonProcessingException e) {
                log.error(e.getMessage(), e);
            }
