...
public class RabbitMqAnalyzer implements ClassAnalyzer {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RabbitMqAnalyzer.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
        ...
        for (final Method method : methods) {
            if (ReflectionService.hasAnnotation(method, RabbitListener.class, RabbitHandler.class)) {
                ...
                buildSenderNode(method, (node, properties) -> {
                    ...
                    final Node methodNode = buildMethodNode(clazz, method);
                    result.addNode(methodNode);
                    result.addRelationship(buildRelationships(node, CONSUME, projectNode, serviceType, methodNode, properties, "produce"));
                });
            }
        }
    ...
    private void buildSenderNode(final Method method, final BiConsumer<Node, List<Node>> onData) {
        ...
        if (hasText(sender.id())) {
            ...
            final String prefix = notNullValue(sender.echangeName()).isEmpty() ? notNullValue(sender.queue()) : notNullValue(sender.echangeName());
            uid = String.join("_", cleanValue(prefix), cleanValue(notNullValue(sender.routingKey())));
        }
        final Parameter event = resolveRabbitEvent(method);
        if (event == null) {
            log.warn("no RabbitMqEvent define on method parameters : {}.{}", method.getDeclaringClass(), method.getName());
        }
        ...
        final Node result = Node.builder().type(SERVICE).name(uid).uid(uid).properties(additionalInfo).build();
    ...
    private Parameter resolveRabbitEvent(final Method method) {
        ...
        if (event == null) {
            for (final Parameter param : method.getParameters()) {
                if (param.getAnnotations() == null || param.getAnnotations().length == 0) {
                    event = param;
                    break;
                }
            }
    ...
    // BUILDERS LISTENER
    // =========================================================================
    private void buildListenerNode(final Method method, final RabbitListener parentRabbitListener, final BiConsumer<Node, List<Node>> onData) {
        ...
        if (handlerInfo == null && log.isDebugEnabled()) {
            log.warn("cann\'t resolve all rabbitMQ information without RabbitMqHandlerInfo on method : {},{}", method.getDeclaringClass(), method.getName());
        }
        final Parameter event = resolveRabbitEvent(method);
    ...
    private String resolveUid(final RabbitListener rabbitListener, final RabbitMqHandlerInfo handlerInfo) {
        ...
        if (rabbitListener != null) {
            for (final QueueBinding queueBinding : Optional.ofNullable(rabbitListener.bindings()).orElse(new QueueBinding[] {})) {
                ...
                for (final String key : Optional.ofNullable(queueBinding.key()).orElse(new String[] {})) {
        ...
        return String.join("_", prefix, routingKey == null ? String.join(";", routingKeys) : cleanValue(routingKey));
    }
    ...
    private List<Relationship> buildRelationships(final Node jmsNode, final String jmsNodeType, final Node artifactNode, final Node serviceNode, final Node methodNode, final List<Node> properties, final String useByType) {
        final List<Relationship> result = new ArrayList<>();
        result.add(Relationship.builder().from(artifactNode.getUid()).to(jmsNode.getUid()).type(jmsNodeType).build());
        result.add(Relationship.builder().from(artifactNode.getUid()).to(methodNode.getUid()).type(BuilderTools.RELATION_HAS_METHOD).build());
        result.add(Relationship.builder().from(jmsNode.getUid()).to(methodNode.getUid()).type(BuilderTools.RELATION_USE_BY).properties(new LinkedHashMap<>(Map.of("linkType", useByType))).build());
        result.add(Relationship.builder().from(jmsNode.getUid()).to(serviceNode.getUid()).type(SERVICE_TYPE_RELATIONSHIP).build());
        if (properties != null) {
    ...
    private String buildEventPayload(final Parameter parameter) {
        ...
        if (ReflectionService.hasAnnotation(parameter, RabbitMqEvent.class) && parameter.getAnnotation(RabbitMqEvent.class).value() != RabbitMqEvent.None.class) {
            result = renderType(parameter.getAnnotation(RabbitMqEvent.class).value(), null, null);
        } else {
