...
public class PublishInfoKeeper implements ProjectPreAnalyzer, QueryConfigurator {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PublishInfoKeeper.class);
    // =========================================================================
    // QUERIES
    // =========================================================================
    ...
    private void savePublishInfo(final Record record, final ScanConext context) {
        ...
        if (version != null && env != null) {
            backup.addRelationship(Relationship.builder().from(version).to(env).type(Publish.DEPLOY).properties(extractProperties(deploy)).build());
            backup.addRelationship(Relationship.builder().from(env).to(version).type(Publish.HAVE_ARTIFACT_VERSION).properties(extractProperties(haveArtifactVersion)).build());
        }
    }
