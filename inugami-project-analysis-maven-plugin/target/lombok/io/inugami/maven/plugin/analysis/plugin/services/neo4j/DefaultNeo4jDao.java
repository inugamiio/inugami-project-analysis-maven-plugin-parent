...
public class DefaultNeo4jDao implements Neo4jDao {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DefaultNeo4jDao.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    public DefaultNeo4jDao(final ConfigHandler<String, String> configuration) {
        encoders = SpiLoader.INSTANCE.loadSpiServicesWithDefault(Neo4jValueEncoder.class, new DefaultNeo4jEncoder());
        driver = GraphDatabase.driver(configuration.grab("inugami.maven.plugin.analysis.writer.neo4j.url"), AuthTokens.basic(configuration.grab("inugami.maven.plugin.analysis.writer.neo4j.user"), decodePassword(configuration)));
    }
    ...
    public String searchQuery(final String name, final String type) {
        return new StringBuilder().append("MATCH (n:").append(type).append(") where n.name=\"").append(name).append("\" return n").toString();
    }

    @Override
    public String buildDeleteNodeQuery(final String uid) {
        return new JsonBuilder().write("MATCH (n) where n.name=").valueQuot(uid).write(" detach delete n").toString();
    }

    @Override
    public String buildCreateNodeQuery(final io.inugami.maven.plugin.analysis.api.models.Node node, final Map<String, Object> parameters) {
    ...
    public String buildRelationshipQuery(final Relationship relationship) {
        ...
        if (relationship.getProperties() != null && !relationship.getProperties().isEmpty()) {
            query.append(" {");
            boolean first = true;
            final Iterator<Map.Entry<String, Serializable>> iterator = relationship.getProperties().entrySet().iterator();
            while (iterator.hasNext()) {
