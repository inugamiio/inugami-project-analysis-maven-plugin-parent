...
public class QueryDisplay implements ProjectInformation {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(QueryDisplay.class);
    ...
    public void process(final InfoContext context) {
        ...
        if (queryName == null) {
            ...
            final List<QueryConfigurator> configurators = SpiLoader.INSTANCE.loadSpiServicesByPriority(QueryConfigurator.class);
            final QueryConfigurator configurator = configurators.stream().filter(item -> item.accept(queryDef.getPath())).findFirst().orElse(null);
            ConfigHandler<String, String> currentConfiguration = context.getConfiguration();
            if (configurator != null) {
                currentConfiguration = configurator.configure(queryDef.getPath(), convertMavenProjectToGav(context.getProject()), context.getConfiguration());
            }
