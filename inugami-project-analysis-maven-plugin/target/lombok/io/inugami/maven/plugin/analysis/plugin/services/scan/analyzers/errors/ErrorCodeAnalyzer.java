...
public class ErrorCodeAnalyzer implements ClassAnalyzer {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ErrorCodeAnalyzer.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    private List<JsonObject> processAnalyze(final Class<?> clazz, final ScanConext context) {
        ...
        if (nodes != null && !nodes.isEmpty()) {
            ...
            for (final Node node : nodes) {
                ...
                result.addRelationship(Relationship.builder().from(artifactNode.getUid()).to(node.getUid()).type(HAS_ERROR_CODE).build());
                result.addRelationship(Relationship.builder().from(node.getUid()).to(errorType.getUid()).type(HAS_ERROR_TYPE).build());
            }
    ...
    private Node buildNode(final Object instance) {
        ...
        for (final Method method : methods) {
            ...
            if (method.getParameters().length == 0 && !errorCodeClass.isAssignableFrom(method.getReturnType())) {
                try {
                    ...
                    if (log.isDebugEnabled()) {
                        log.error("instance : {} on retrieve value from method : {}", instance.getClass(), method.getName());
                        log.error(e.getMessage(), e);
                    }
                }
            }
        ...
        final String uid = properties.get(errorCodeFieldName) == null ? instance.toString() : String.valueOf(properties.get(errorCodeFieldName));
    ...
    private Node buildErrorType(final Node errorNode) {
        final String errorType = errorNode.getProperties().get("errorType") == null ? "technical" : String.valueOf(errorNode.getProperties().get("errorType"));
        return Node.builder().type(ERROR_TYPE).uid(ERROR_TYPE_PREFIX + errorType).name(errorType).build();
    }
