...
public class SpringRestControllersAnalyzer implements ClassAnalyzer {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SpringRestControllersAnalyzer.class);
    ...
    public List<JsonObject> analyze(final Class<?> clazz, final ScanConext context) {
        ...
        if (!result.getNodes().isEmpty()) {
            ...
            for (final Node service : result.getNodes()) {
                result.addRelationship(Relationship.builder().from(node.getUid()).to(service.getUid()).type(getRelationshipType()).build(), Relationship.builder().from(service.getUid()).to(serviceType.getUid()).type(SERVICE_TYPE_RELATIONSHIP).build());
            }
            result.addNode(node, serviceType);
        }
        if (!existingNodes.isEmpty()) {
            for (final String service : existingNodes) {
                result.addRelationship(Relationship.builder().from(node.getUid()).to(service).type(getRelationshipType()).build());
            }
        }
        return List.of(result);
    }

    private boolean existingNode(final Node node, final Neo4jDao neo4jDao) {
    ...
    private Node convertEndpointToNeo4j(final RestEndpoint endpoint) {
        ...
        return Node.builder().uid(encodeSha1(uid)).name(buildName(endpoint)).type(SERVICE).properties(buildProperties(endpoint, uid)).build();
    }
    ...
    private String buildServiceUid(final RestEndpoint endpoint) {
        ...
        //@formatter:on
        return json.toString().replaceAll(LINE, EMPTY).replaceAll(TAB, EMPTY).replaceAll(DOUBLE_URL_SEP, URI_SEP).replaceAll(QUOT, SIMPLE_QUOT);
    }
    ...
    private LinkedHashMap<String, Serializable> buildProperties(final RestEndpoint endpoint, final String identifier) {
    ...
    protected RestApi analyseClass(final Class<?> clazz) {
        ...
        return RestApi.builder().name(name).baseContext(URI_SEP + baseContext).endpoints(resolveEndpoints(clazz, baseContext, true)).build().orderEndPoint();
    }
    ...
    private List<RestEndpoint> resolveEndpoints(final Class<?> clazz, final String baseContext, final boolean strict) {
        ...
        for (final Method method : clazz.getMethods()) {
            if (hasAnnotation(method, RequestMapping.class, GetMapping.class, PostMapping.class, PutMapping.class, DeleteMapping.class)) {
