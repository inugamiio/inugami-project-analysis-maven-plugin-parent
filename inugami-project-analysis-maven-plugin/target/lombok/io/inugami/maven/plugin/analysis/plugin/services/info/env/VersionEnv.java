...
public class VersionEnv implements ProjectInformation, QueryConfigurator {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(VersionEnv.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    // QUERIES
    // =========================================================================
    private static final List<String> QUERIES = List.of("META-INF/queries/search_deploy_artifact.cql", "META-INF/queries/search_missing_service.cql");
    ...
    public void process(final InfoContext context) {
        ...
        final String query = TemplateRendering.render(QueriesLoader.getQuery(QUERIES.get(0)), configure(QUERIES.get(0), gav, context.getConfiguration()));
        log.info("query:\n{}", query);
        final Map<String, Long> envs = new LinkedHashMap<>();
        final Map<String, Collection<DataRow>> firstPass = extractDataFromResultSet(dao.search(query), (data, record) -> this.buildModels(data, record, envs));
        final Map<String, Collection<DataRow>> data = sortData(firstPass, envs, extractProjectEnvs(firstPass, gav));
        final String queryMissingService = TemplateRendering.render(QueriesLoader.getQuery(QUERIES.get(1)), configure(QUERIES.get(1), gav, context.getConfiguration()));
        final Map<String, Collection<DataRow>> missingServices = extractDataFromResultSet(dao.search(queryMissingService), this::buildMissingService);
    ...
    // BUILDER
    // =========================================================================
    public void buildModels(final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final Map<String, Long> envs) {
    ...
    private void buildCurrentProjectEnv(final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final Map<String, Long> envs) {
        buildLine("version", "env", "deploy", data, record, ConsoleColors.CYAN, envs);
    }

    private void buildProjectDependenciesEnv(final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final Map<String, Long> envs) {
        buildLine("dep", "envDep", "deployDep", data, record, envs);
    }

    private void buildProjectConsumersEnv(final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final Map<String, Long> envs) {
        buildLine("depProducer", "envProducer", "deployProducer", data, record, envs);
    }

    public void buildLine(final String artifactNodeName, final String envNodeName, final String relationshipName, final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final Map<String, Long> envs) {
        buildLine(artifactNodeName, envNodeName, relationshipName, data, record, null, envs);
    }

    public void buildLine(final String artifactNodeName, final String envNodeName, final String relationshipName, final Map<String, Collection<DataRow>> data, final Map<String, Object> record, final String color, final Map<String, Long> envs) {
        ...
        if (envProperties != null) {
            final Long level = envProperties.get("level") instanceof Long ? (Long) envProperties.get("level") : 0L;
    ...
    // MISSING SERVICE
    // =========================================================================
    private void buildMissingService(final Map<String, Collection<DataRow>> data, final Map<String, Object> record) {
    ...
    // SORT
    // =========================================================================
    public Map<String, Collection<DataRow>> sortData(final Map<String, Collection<DataRow>> firstPass, final Map<String, Long> envs, final List<String> projectEnv) {
            ...
            for (final Map.Entry<String, Collection<DataRow>> entry : data.entrySet()) {
                for (final DataRow row : entry.getValue()) {
                    if (name.equals(row.getUid())) {
                        final Set<String> envs = Optional.ofNullable(row.getProperties()).orElse(new LinkedHashMap<>()).keySet();
