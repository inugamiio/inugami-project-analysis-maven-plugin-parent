...
public class ReleaseNoteAsciidocWriter implements ReleaseNoteWriter {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ReleaseNoteAsciidocWriter.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    private static final Pattern COMMIT_REGEX = Pattern.compile("(?:\\[([^]]+)\\])(?:\\[([^]]+)\\])(?:\\[([^]]+)\\])(.*)");
    ...
    // OVERRIDES
    // =========================================================================
    private File buildBaseDocFolder(final File basedir, final ConfigHandler<String, String> configuration) {
        final String path = configuration.getOrDefault(BASE_DOC_FOLDER, FilesUtils.buildPath(basedir, "src", "doc", "releases"));
    ...
    // RENDERING
    // =========================================================================
    private void render(final ReleaseNoteResult releaseNote, final File baseDocFolder, final InfoContext context) throws IOException {
        ...
        final List<AsciidocInfoWriter> infoWriters = SpiLoader.INSTANCE.loadSpiServicesByPriority(AsciidocInfoWriter.class);
        for (final AsciidocInfoWriter writerInfo : infoWriters) {
            if (writerInfo.isEnabled(context.getConfiguration())) {
                ...
                if (content != null) {
                    if (content.size() == 1) {
                        write(content.get(new ArrayList<>(content.keySet()).get(0)), writer, writerInfo.getParagraphName(), version, baseDocFolder);
                    } else {
                        for (final Map.Entry<String, String> entry : content.entrySet()) {
                            write(entry.getValue(), writer, String.join(Constants.UNDERSCORE, writerInfo.getParagraphName(), entry.getKey()), version, baseDocFolder);
                        }
                    }
                }
            }
    ...
    private void write(final String content, final Writer writer, final String context, final String version, final File baseDocFolder) throws IOException {
        if (content != null) {
            if (writer != null) {
                writer.write(content);
                writer.flush();
            } else {
                final File file = FilesUtils.buildFile(baseDocFolder, version, String.join(DELIMITER, RELEASE_NOTE, context + ADOC));
                FilesUtils.write(content, file);
            }
        }
    }

    private Writer buildWriter(final File baseDocFolder, final String version, final boolean notSplitFile) throws IOException {
        Writer writer = null;
        if (notSplitFile) {
            final File file = FilesUtils.buildFile(baseDocFolder, String.join(DELIMITER, RELEASE_NOTE, "" + version + ADOC));
            if (file.exists()) {
                file.delete();
            }
    ...
    private String renderProject(final MavenProject project, final boolean notSplitFile) {
        ...
        writer.write(project.getGroupId()).write(":").write(project.getArtifactId()).write(":").write(project.getVersion());
    ...
    protected String renderAuthors(final Set<Author> authors, final boolean notSplitFile, final ConfigHandler<String, String> configuration) {
        ...
        if (authors != null) {
            ...
            currentAuthors.sort((ref, value) -> {
                return String.valueOf(ref == null ? null : ref.getName()).compareTo(value == null ? null : value.getName());
            });
            if (notSplitFile) {
    ...
    protected String renderCommit(final Set<String> commit, final boolean notSplitFile, final ConfigHandler<String, String> configuration) {
    ...
    protected String renderMergeRequest(final List<MergeRequests> mergeRequests, final boolean notSplitFile, final ConfigHandler<String, String> configuration) {
        ...
        if (mergeRequests != null) {
            ...
            data.sort((ref, value) -> {
                return String.valueOf(ref == null ? null : ref.getDate()).compareTo(String.valueOf(value == null ? null : value.getDate()));
            });
    ...
    protected String renderIssues(final List<Issue> values, final boolean notSplitFile, final ConfigHandler<String, String> configuration) {
