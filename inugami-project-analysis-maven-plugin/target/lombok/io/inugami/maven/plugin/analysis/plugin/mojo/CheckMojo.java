...
public class CheckMojo extends AbstractMojo {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CheckMojo.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    @Parameter
    ...
    private void preAnalyze(final ScanConext context) {
        ...
        final List<ProjectPreAnalyzer> scanners = SpiLoader.INSTANCE.loadSpiServicesByPriority(ProjectPreAnalyzer.class);
    ...
    private void postAnalyse(final ScanConext context, final List<JsonObject> result) {
        log.info("launching pre analyze ...");
        final List<ProjectPostAnalyzer> scanners = SpiLoader.INSTANCE.loadSpiServicesByPriority(ProjectPostAnalyzer.class);
    ...
    private ScanConext buildContext(final ArtifactResolverListener listener) throws MojoExecutionException {
        ...
        configuration.put(Constants.PROJECT_BUILD_DIR, FilesUtils.buildFile(project.getBasedir(), "target").getAbsolutePath());
        ...
        return ScanConext.builder().basedir(basedir).project(project).repoSystem(repoSystem).repoSession(repoSession).repositories(repositories).classLoader(buildClassloader(listener, configuration)).dependencies(listener.getArtifacts()).directDependencies(extractDirectDependencies()).pluginDescriptor(pluginDescriptor).configuration(configuration).secDispatcher(secDispatcher).artifactHandler(artifactHandler).settings(settings).neo4jDao(neo4jDao).build();
    }
    ...
    private void initProperties(final ConfigHandler<String, String> configuration) {
        ...
        if (secDispatcher instanceof DefaultSecDispatcher) {
            final String securityPath = configuration.getOrDefault("settings.security", new File(System.getProperty("user.home") + File.separator + ".m2/settings-security.xml").getAbsolutePath());
            ((DefaultSecDispatcher) secDispatcher).setConfigurationFile(securityPath);
        }
        final List<PropertiesInitialization> propertiesInitializers = SpiLoader.INSTANCE.loadSpiServicesByPriority(PropertiesInitialization.class);
    ...
    private Set<Gav> extractDirectDependencies() {
        final List<Gav> dependenciesGav = Optional.ofNullable((List<Dependency>) project.getDependencies()).orElse(new ArrayList<>()).stream().map(this::convertToGav).collect(Collectors.toList());
        return new LinkedHashSet<>(dependenciesGav);
    }

    private Gav convertToGav(final Dependency dependency) {
        return Gav.builder().groupId(dependency.getGroupId()).artifactId(dependency.getArtifactId()).version(dependency.getVersion()).type(dependency.getType()).build();
    }

    private JarClassLoader buildClassloader(final ArtifactResolverListener listener, final ConfigHandler<String, String> configuration) throws MojoExecutionException {
        try {
    ...
    private JarClassLoader buildDependenciesClassLoader(final ArtifactResolverListener listener) {
        ...
        final Set<Artifact> artifacts = repoSystem.resolve(new ArtifactResolutionRequest().setArtifact(this.project.getArtifact()).setLocalRepository(artifactRepository).setRemoteRepositories(this.project.getRemoteArtifactRepositories()).setResolveTransitively(true).addListener(listener)).getArtifacts();
