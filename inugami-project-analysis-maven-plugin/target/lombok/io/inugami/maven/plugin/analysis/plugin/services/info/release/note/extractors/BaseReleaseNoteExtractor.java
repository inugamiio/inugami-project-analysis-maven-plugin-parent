...
public class BaseReleaseNoteExtractor implements ReleaseNoteExtractor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(BaseReleaseNoteExtractor.class);
    // =========================================================================
    // ATTRIBUTES
    // =========================================================================
    ...
    public void extractInformation(final ReleaseNoteResult releaseNoteResult, final Gav currentVersion, final Gav previousVersion, final Neo4jDao dao, final List<Replacement> replacements, final InfoContext context) {
        ...
        config.putAll(Map.ofEntries(Map.entry(GROUP_ID, currentVersion.getGroupId()), Map.entry(ARTIFACT_ID, currentVersion.getArtifactId()), Map.entry(VERSION, currentVersion.getVersion())));
        final String query = TemplateRendering.render(QueriesLoader.getQuery(QUERIES_SEARCH_RELEASE_NOTE_SIMPLE_CQL), config);
        log.info("query:\n{}", query);
    ...
    // OVERRIDES
    // =========================================================================
    private void convertToReleaseNote(final ReleaseNoteResult releaseNote, final List<Record> resultSet, final List<Replacement> replacements) {
    ...
    private void convertReleaseNote(final ReleaseNoteResult releaseNote, final Record record, final Map<String, Serializable> cache, final List<Replacement> replacements) {
        //@formatter:off
    ...
    private void addScm(final ReleaseNoteResult releaseNote, final NodeValue scm, final Map<String, Serializable> cache, final List<Replacement> replacements) {
    ...
    private void addMergeRequest(final ReleaseNoteResult releaseNote, final NodeValue node, final Map<String, Serializable> cache) {
        ...
        releaseNote.addMergeRequest(MergeRequests.builder().uid(retrieveString(SHORT_NAME, data)).title(retrieveString(TITLE, data)).date(retrieveString(MERGED_AT, data)).url(retrieveString(URL, data)).build());
    }

    private void addIssues(final ReleaseNoteResult releaseNote, final NodeValue nodeValue, final NodeValue labelNode) {
        Issue savedIssue = null;
        if (isNotNull(nodeValue)) {
            final Map<String, Object> data = nodeValue.asMap();
            final Issue issue = Issue.builder().name(retrieveString(SHORT_NAME, data)).title(retrieveString(TITLE, data)).date(retrieveString(CREATED_AT, data)).url(retrieveString(URL, data)).build();
    ...
    private void addAuthors(final ReleaseNoteResult releaseNote, final NodeValue node, final Map<String, Serializable> cache, final List<Replacement> replacements) {
        ...
        cache.put(cacheKey, Boolean.TRUE);
        final Map<String, Object> data = node.asMap();
        releaseNote.addAuthor(Author.builder().name(retrieveString(SHORT_NAME, data, replacements)).email(retrieveString(Constants.EMAIL, data, replacements)).build());
    }
